...
- **`docs/architecture.md` corrected.** It still claimed "no player-facing write (kick, ban, grant item) exists today" and named `src/writeCommands.js` as the real write-command list; player-facing write commands ship in this branch and the source of truth is `src/writeActions.js`. (`scripts/check-architecture-doc-drift.js` still only parses the superseded `writeCommands.js` — widening it is tracked separately, #401.)
