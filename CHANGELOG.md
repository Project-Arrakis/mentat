...
- **27 real Discord write commands** wired to Core's write bridge (`dune-awakening-selfhost-docker`#215/#1026): player moderation (kick/ban/unban/warn/give-item/clear-backpack/fill-water), base management, server control (restart/stop/start/restart-service — stop is designed to require a second, different owner-tier admin to confirm, though see the known issue below), map control, care packages, guild membership, and operations (backup/game-update/steamcmd-fix). Overrides `docs/upstream-write-adapter-rfc.md` and `docs/r1-r2-release-roadmap.md`'s prior "do not implement" gates for this scope — see #398.
...
- **Any guild member could destroy a pending dual confirmation.** The `server.stop` waiting-state message is deliberately public so a *different* admin can complete it; the button handler had no way to check the clicker's tier (it was never given `config`/`db`). Confirm clicks were refused by Core but only after the pending entry had already been cleared, and Cancel clicks had no authorization check at all. `handleWriteButtonInteraction(interaction, adapterClient, config, db)` now verifies the clicker actually holds the action's tier via `canWrite()` and leaves the pending entry untouched when they don't.
- **Audit gaps closed:** the dual-confirmation waiting-state transition (`second-confirmation-required`), rejected second-confirmation attempts (`second-confirmation-denied`), expiry of a real/self-update pending confirmation (`timeout` — previously only the legacy stub path audited this), and both outcomes of a bot self-update (`self-update-completed` from the restarted process's marker-file check, `self-update-aborted` emitted by `scripts/self-update.sh` itself, since on that path the old process survives and can never learn the outcome).
...
### Known issue introduced by the fix above

- **`server.stop`'s dual-confirmation second step is currently unsatisfiable.** Fixing the "any guild member could destroy a pending confirmation" gap above added a real, local tier check to the second confirmer's click — but this bot derives owner tier *exclusively* from real Discord guild ownership (exactly one account per guild), and `server.stop`'s dual-confirmation tier is `owner`, so no genuinely different second admin can ever pass that check. Core's own tier model has a second, role-based owner path this bot doesn't grant owner through, which is what made the flow satisfiable before this fix (via a role-mapped, non-owner "owner"). This is a fail-closed regression (the action gets harder to trigger, never easier — no security exposure), not a silent one: `/dune server stop` is unaffected in terms of safety, just currently unusable via this specific dual-confirmation path. `server stop` remains reachable through the web console and the `dune` CLI. Tracked in #404 for a deliberate design decision (relax the second step to admin tier, drop dual confirmation for this action, or reconcile the bot's and Core's owner models).
