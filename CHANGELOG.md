...
- **27 real Discord write commands** wired to Core's write bridge (`dune-awakening-selfhost-docker`#215/#1026): player moderation (kick/ban/unban/warn/give-item/clear-backpack/fill-water), base management, server control (restart/stop/start/restart-service — `stop` is owner-tier with a single confirmation, like every other write action), map control, care packages, guild membership, and operations (backup/game-update/steamcmd-fix). Overrides `docs/upstream-write-adapter-rfc.md` and `docs/r1-r2-release-roadmap.md`'s prior "do not implement" gates for this scope — see #398.
...
### Fixed (final whole-branch review, write-command reconciliation)

Cross-task integration defects found by the final whole-branch review, after every individual task had already passed its own review:

- **Confirmed writes were executed against the wrong tenant's Core.** `writeConfirmation.js`'s confirm branch called `adapterClient.writeExecute(actor, body)` without the third `guildId` argument, so in multi-tenant mode every confirmed write resolved the process-wide default adapter config instead of the clicking guild's own — preview (which did pass it) and execute could target two different Core instances for the same command.
- **The production deploy hook could not load its own shared library.** `scripts/deploy-post-receive.sh` sourced `scripts/lib/deploy-core.sh` relative to `${BASH_SOURCE[0]}`, but the hook is deployed by being *copied* into the bare deploy repo's `hooks/` directory (`INSTALL.md`), where that path does not exist; with `set -u` and no `-e` the failed source was non-fatal and every deploy then died on an undefined `deploy_core::sync_test_install_restart`. Now `$WORK_DIR`-relative, matching every other script reference in the same file, with a new `test/deploy-hook.bats` case that runs the hook from a real copied-out `hooks/post-receive`.
- **A pending confirmation could be destroyed by someone it did not belong to.** Confirm clicks from another user were refused by Core, but only after the pending entry had already been cleared, and Cancel clicks had no authorization check at all. A write confirmation button is now actionable only by the actor who requested it, for both Confirm and Cancel, and a refused click leaves the pending entry untouched. (Originally fixed with a `canWrite()` tier re-check scoped to `server.stop`'s public dual-confirmation window; that window no longer exists — see "Removed" below — so the plain ownership gate covers it.)
- **Audit gaps closed:** expiry of a real/self-update pending confirmation (`timeout` — previously only the legacy stub path audited this), and both outcomes of a bot self-update (`self-update-completed` from the restarted process's marker-file check, `self-update-aborted` emitted by `scripts/self-update.sh` itself, since on that path the old process survives and can never learn the outcome).
- **A malformed `expiresAt` from Core silently deleted a confirmation immediately.** `setTimeout(fn, NaN)` fires on the next tick; `registerRealPendingConfirmation()` now validates the value and falls back to the local confirmation window.
- **Wrong tier named in denial messages.** A denied `player.warn` (the only moderator-tier write action) said "Requires admin-tier access", sending operators off to grant a strictly higher tier than the action needs.
- **`docs/architecture.md` corrected.** It still claimed "no player-facing write (kick, ban, grant item) exists today" and named `src/writeCommands.js` as the real write-command list; player-facing write commands ship in this branch and the source of truth is `src/writeActions.js`.

### Fixed (follow-up batch, #397/#400/#401/#403)

- **#397: the self-update marker file (`runtime/self-update-pending.json`) had no explicit file mode**, landing at the process's default umask instead of 0600 like the short-lived webhook-secret temp file it sits alongside — both embed the same bearer-style Discord interaction token. Now written with `{ mode: 0o600 }`.
- **#400: deleted `src/writeCommands.js`**, the original hand-written write-command registry — it had zero real importers left anywhere in `src/`/`test/` once `writeActions.js`/`writeHandler.js` became the single source of truth, and its own bug (`executeWriteCommand` called `canWrite()` with no required tier, letting admin-tier reach owner-tier actions) is moot with nothing left to call it.
- **#401: `scripts/check-architecture-doc-drift.js` read the now-dead `src/writeCommands.js`** as its sole source of truth for `docs/architecture.md`'s write-command drift check, giving a false "no drift detected" the entire time the doc's write-command claims went stale. Now reads `src/writeActions.js`/`src/writeHandler.js`. Running the corrected checker immediately found real, previously-hidden drift (`docs/architecture.md`'s legacy-command names used the old `set-maintenance-note`-style naming instead of the real `maintenance-note`-style names) — fixed in the same pass.
- **#403: two more independently hand-maintained copies of the 9 legacy `write` group commands' descriptions had drifted** from `writeHandler.js`'s `LEGACY_WRITE_STUBS` (the intended source of truth) — `commands.js`'s own Discord command builder (7 of 9 descriptions stale) and `commands.js`'s `WRITE_HELP_ENTRIES` (consumed by `/dune help`; also still advertised `write:backup`/`write:restart`/`write:update`, three commands removed from registration entirely during write-command reconciliation and no longer typeable). Both aligned to `LEGACY_WRITE_STUBS`'s text; new tests cross-check all three copies stay in sync going forward.

### Removed

- **`server.stop`'s dual-confirmation requirement, and all of the machinery that supported it (#404).** `server.stop` was the only action anywhere in `src/writeActions.js` carrying `requiresDualConfirmation: true`, and its second step was structurally unsatisfiable: this bot derives owner tier *exclusively* from real Discord guild ownership (exactly one account per guild), so "a second, **different** owner-tier admin" could never exist and `/dune server stop` was unusable through Discord. The flag is now removed on both sides (Core's route table drops it in parallel), making `stop` a single-confirmation, owner-tier write like every other one. Removed with it, since nothing can ever set the flag again: the `secondConfirmationPending` entry field, the ownership-gate exception and its `canWrite()` tier re-check for a second clicker (and therefore `handleWriteButtonInteraction()`'s `config`/`db` parameters), the same-actor rejection, the "⏳ Waiting on a Second Administrator" waiting state and its nonce re-registration, and the `second-confirmation-required`/`second-confirmation-denied` audit results. Core's write bridge still supports the `second_confirmation_required`/`second_confirmation_same_actor` protocol codes generically, and `src/writeErrorMapping.js` still maps both to real operator-facing messages, so the mechanism can be revived for a future action — but doing so requires restoring the client-side second-step handling in `src/writeConfirmation.js`, not just re-adding the flag.
...
- **#281: `/dune core setup`'s generated invite link was missing `permissions=128` (VIEW_AUDIT_LOG), breaking inviter identification** — the exact same bug already fixed once for the landing-site/docs invite links (see #278), never applied to this bot-generated one. Added a regression test asserting the generated URL contains `permissions=128`; verified it fails without the fix. **Superseded later in this same Unreleased section:** the "Removed the DM-on-invite onboarding trigger" entry above deletes `findInviter()` and the whole invite-attribution flow this fix served — `permissions=128`/View Audit Log is no longer functionally required by anything in this repo as of this release. See `docs/discord-setup.md` for the current, annotated guidance.
...
### Added
- **New `/dune server atlas` command + self-refreshing `#the-atlas` channel message: per-sietch PvP/PvE and live sandstorm status** (mentat#376, companion to `dune-awakening-selfhost-docker`#938/#953, "Chronicles of Kanly" rollout `#the-atlas` channel). Calls Core's new public-tier `world/atlas` route, which combines the same combat-state resolver `/api/maps/combat-state` already uses (PvP/PvE + display name per sietch/Deep-Desert-instance), the farm-wide Coriolis cycle, and a new live sandstorm-detection signal (Core parses each game server container's own stdout log; neither map logs a storm end, so "active" is a documented ~4-minute heuristic window, not a confirmed ongoing storm). **This is the first real caller of `liveMessage.js`'s `postOrEditLiveMessage()`** (`#370` shipped that utility unused, since the Coriolis countdown needed no server-side refresh loop): a new optional `DUNE_ATLAS_CHANNEL_ID` env var enables a periodic job (default every 10 minutes, `DUNE_ATLAS_INTERVAL_MS`) that posts once and edits the same message in place afterward, exactly the "self-refreshing pinned message" `#the-atlas` was designed around. Requires multi-tenant mode (a local database to record the message pointer in) — the live deployment already runs with `ACP_MULTI_TENANT=true`, confirmed directly rather than assumed. Sandworm/storm-cadence config is deliberately not included yet, per Core issue #938's own fast-follow note.
- **New `/dune server coriolis` command: shows the current Coriolis storm seed and next-cycle countdown** (mentat#370, companion to `dune-awakening-selfhost-docker`#942, "Chronicles of Kanly" rollout). Calls Core's new public-tier `world/coriolis` route rather than SSHing into the game-server host directly (the original design's assumption, which contradicted this bot's established architecture of never having direct filesystem/SSH access to game-server hosts). Renders the countdown using Discord's native `<t:UNIX:R>`/`<t:UNIX:F>` relative-timestamp formatting rather than computing "X hours remaining" ourselves, so it stays live-accurate client-side between refreshes with no extra bot-side work. Also adds `src/liveMessage.js` (schema v8, new `live_messages` table) — shared "post a message once, edit it in place on refresh" infrastructure, since nothing in this bot did this before. **Not yet wired to any live caller in this PR** (Layer 3 Architect-hat finding, 2026-09-16): `/dune server coriolis` itself is a plain on-demand read command through the normal one-shot reply pipeline, using Discord's `<t:UNIX:R>` client-side rendering for the countdown, not an edited-in-place message — `postOrEditLiveMessage()` is exercised only by `test/liveMessage.test.js` right now. Built in this PR as new shared infrastructure ready for mentat#369 (Landsraad tracker) and mentat#372 (duty-status embeds) to adopt, whichever lands first; an earlier draft of this changelog entry overstated this as already wired in, corrected here.
