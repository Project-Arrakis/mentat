...
### Added
- **New `/dune admin service-setup` command and the generalized service duty/apply button component** (mentat#372, "Chronicles of Kanly" rollout, companion infrastructure to #370's `live_messages`/`postOrEditLiveMessage()`). Every Chronicles of Kanly service channel (Water-Seller, Smuggler now; the remaining 12 later) gets a pinned status embed with On Duty/Off Duty/Apply buttons, backed by three new additive tables (`service_channels`, `service_duty_status`, `service_applications`, schema v9) and a new `guild_settings.on_duty_role_id` column for the guild's single generic On Duty role (per the plan doc's §8 "one generic role, not one per service"). Applying opens a modal (this bot's first-ever `ModalBuilder`/`ModalSubmitInteraction` usage) and posts a review embed with Approve/Deny buttons to a per-guild-configured applications channel — `proof_link` is rendered as a plain-text code span, never interpolated into markdown link syntax, to prevent a malicious applicant from phishing staff reviewers with a deceptive masked link. Approve/Deny is admin-gated and guild-scoped (mentat is confirmed multi-tenant; an application can only be resolved by an admin of its own guild). Went through a full Requirement 20 Layer 1 design audit before implementation began (findings register on mentat#372); the Layer 2 implementation audit and Layer 3 integration audit for this PR follow before merge, per the same requirement. Deferred to follow-up issues: denial-reason capture UI (#382) and an in-guild application-status fallback for DM-closed applicants (#383).
