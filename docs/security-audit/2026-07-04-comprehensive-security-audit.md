...
| FINDING-CORE-1 Hardcoded command-auth token | **Regressed 2026-07-07.** A separate fix (`security/generated-command-auth-token-fix`) was merged to the core repo's `main` on 2026-06-27/28, then explicitly reverted by upstream maintainer Red-Blink in commit `52008a7` ("Restore built-in command auth token fallback"). The hardcoded fallback is present again on `origin/main`/`upstream/main` (`e188c87`) as of this update. The `security/core-remove-hardcoded-command-token` branch referenced below predates the revert and is not yet rebased past it. | `security/core-remove-hardcoded-command-token` (stale — predates revert) | Not currently passing against `main`; needs re-verification after rebase |
