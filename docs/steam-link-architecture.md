...
- New config: `ACP_STEAM_LINK_PORT` (default `3101`, distinct from
  ...
  `/dune player link-steam` never need to configure it, and the bot must
  start and run normally without it, only refusing that one specific
  command with a clear "not configured" message if it's missing).
...
Discord User          Bot (Express)         Discord API          Core Adapter
    ...
    │                       │ /dune player link-steam interaction          │
...
src/steamLinkStore.js        — state-token storage (reuses the existing
                                SQLite oauth_sessions table shape/pattern
                                from database.js, extended with an
                                interactionToken column; falls back to an
                                in-memory Map with the same TTL semantics
                                in single-tenant mode, where no SQLite db
                                is otherwise opened)
src/commands.js              — new `player:link-steam` subcommand (no options)
...
console/api/src/integrations/discord/adapter.js
  - two new entries in DISCORD_ADAPTER_ROUTES, following the exact existing
    ...
    PLAYERS_ACCOUNTS_RESOLVE_STESAM: "/api/integrations/discord/players/accounts/resolve-steam"
...
- `src/setupServer.js` — existing Express + Discord OAuth pattern this feature extends
- `src/database.js` — existing `oauth_sessions` table shape (state/token/expiry) this feature's state-store mirrors
