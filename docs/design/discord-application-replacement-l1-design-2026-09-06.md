...
6. **Re-invite the bot to the Discord guild** under the new client ID. **Correction (mentat#278, found while fixing this document's own stale client-ID references): the claim two sentences below this one — that `setupPayload()`'s `permissions=0` link "is the exact scope the old application was invited with" and is "correct and sufficient" — is WRONG, and directly contradicts this same repo's own live-verified history in `src/onboarding.js`'s `handleGuildCreate()` comment block (2026-07-27 live verification).** That comment documents a real, previously-shipped production bug: a `permissions=0` invite link (exactly what `setupPayload()` still generates today) makes the bot's own `guild.fetchAuditLogs()`-based inviter-identification (`findInviter()`) silently fail for every real invite, because it lacks `VIEW_AUDIT_LOG`. That bug was found and fixed once already for the landing site's own "Add to Discord" link (now `permissions=128`, matching `README.md`/`docs/admin-guide.md`/`docs/quick-start-guide.md`/`docs/installation-guide.md` — all four also just corrected in the same PR as this note to point at the new `client_id`) — but `setupPayload()`'s own generated link was never fixed to match, and still isn't (filed as `mentat#281`, not fixed here — out of scope for a docs/incident-record correction). **Practical instruction for this step, until #281 lands:** use one of the four doc files' corrected link (`permissions=128`), NOT `/dune core setup`'s own generated link — the original text below is left visible, struck through in spirit but not in fact (see this correction), as a record of what this document incorrectly asserted at the time #278 was fixed.
   - ~~`[R2]` — use the bot's own generated invite link, not a manually-built one. The audit's Cloud Security hat found a real scope-parity gap: `/dune core setup`'s embed (`src/commands.js`'s `setupPayload()`) generates `https://discord.com/oauth2/authorize?client_id=<id>&scope=bot%20applications.commands` with no `permissions` parameter — this is the exact scope the *old* application was invited with (RBAC is enforced entirely in-app via Discord role IDs, not Discord permission bits, so `permissions=0` is correct and sufficient). Do not use Discord Developer Portal's own OAuth2 URL Generator to hand-build an invite — that flow lets an operator freely tick arbitrary permission checkboxes, which could silently over-grant (Elevation of Privilege) or under-grant (breaks a working feature) relative to what the old application had. Fetch the generated link from the new application's own `/dune core setup` output once step 5 confirms it's online, or construct it manually using the exact template above with the new `client_id` substituted if the command isn't reachable yet.~~
...
**Status as of mentat#278 (2026-09-07):** the cutover itself happened 2026-09-07 ~02:06 UTC, ahead of this checklist ever being marked up — the items below reflect what has since been independently re-verified against the live system (not assumed from the operator's own report) versus what genuinely remains outstanding. See the incident record's own updated Status/Verify sections for the credential-rotation side of this same picture.

- [ ] Old token/secret confirmed reset in Discord Developer Portal (both, unconditionally — §3.1) — bot token rotation is corroborated by the live system now running under the new identity; **the client secret's rotation is NOT independently confirmed** (see incident record)
- [x] New secret files written to the existing `_FILE` paths on the VM, `0600 bot:bot` — implied by `acp-bot.service` running successfully under the new identity (a `readSecret()` throw would prevent startup); file permissions themselves not independently re-checked
- [x] `.env`'s `DISCORD_CLIENT_ID` updated — independently verified directly against the VM (`ssh bot@192.168.22.10`): `DISCORD_CLIENT_ID=1546203607807041697`
- [x] `acp-bot.service` restarted, clean `journalctl` output, no `readSecret` throw — independently verified: service `active`
- [ ] Bot re-invited using its own generated invite link (not a manually-built one) — §3.6 — **not independently verifiable from here** (requires the Discord guild's own member list); also see the §3 step 6 correction above — the *original* instruction here was wrong regardless of whether it was followed
- [ ] `npm run register` run against the new identity — §3.7 — not independently verified (would require querying Discord's own registered-commands API from the VM)
- [ ] Both live-test commands pass (local-only + adapter/RBAC round-trip) — §3.8 — not independently verified
- [ ] Old token's actual invalidation checked (not just assumed) — §3.8 — not independently verified; this is the same still-open item the incident record's Verify section flags
- [ ] Old application's guild membership removed — §3.9 — not independently verifiable from here
- [x] Four public-facing invite-link references updated (§3.1) in the same PR — done (mentat#278); surfaced `mentat#281` as a related, still-open finding
