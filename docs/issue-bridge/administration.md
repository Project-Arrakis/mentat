...
- [ ] The App is installed on exactly `Project-Arrakis/mentat-adjutant` and
      `Project-Arrakis/mentat` — not "all repositories."
- [ ] Public repo: Issue Forms exist under `.github/ISSUE_TEMPLATE/`,
      Discussions enabled (already true — verified via `gh api
      repos/Project-Arrakis/mentat-adjutant --jq .has_discussions`).
- [ ] Private repo: label taxonomy present (already bootstrapped live —
      `gh label list --repo Project-Arrakis/mentat`).
...
1. `gh issue list --repo Project-Arrakis/mentat --label sync:error`
