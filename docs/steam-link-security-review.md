...
- **Risk:** `setupServer.js`'s existing OAuth implementation (which this
  ...
  Discord user who ran `/dune player link-steam` in the first place. Discord
...
- **Recommendation (required before merge):**
  ...
  3. `state` rows must store the originating `discordUserId` and
     `interactionToken` captured at `/dune player link-steam` time. The
     callback handler does not need to re-verify the *browser's* identity
     against this (the browser never authenticated to the bot directly) —
     it only needs the `state` to correctly route the eventual link/edit
     back to the right Discord interaction, which the existing
     `oauth_sessions`-style row shape already supports.
...
- **Risk:** None to the bot itself — this is a deployment/documentation
  ...
  **backward-compatibility-relevant** config change: an operator who
  upgrades and wants to use `/dune player link-steam` must now register a
  redirect URI in the Discord Developer Portal and provision
...
- **Recommendation:** The bot must start and run **normally** without this
  ...
  opt-in-by-absence design in FINDING-LINK-1). Only `/dune player link-steam`
  itself should fail, with a clear "Steam linking is not configured on this
  server" message, if the secret is absent. This must be tested explicitly
  (a startup test with the secret unset, asserting the bot still starts and
